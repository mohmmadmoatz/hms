           ...
           <div class="row">
            ...
            <table class="table table-bordered" dir="ltr">
                <thead>
                    ...
                    <tr>
                        <th> <input type="text" class="form-control" wire:change="changekey('name',$event.target.value,{{$loop->index}})"> </th>
                        <th>
                            ...
                            @if($item['result_type'] == "select")
                              ...
                              @foreach($item['options'] as $opt)
                                <input type="text" class="form-control" wire:change="changesub($event.target.value,{{$loop->index}},{{$loop->parent->index}})">
                                <button wire:click.prevent="deleteopt({{$loop->index}},{{$loop->parent->index}})" class="btn btn-danger"> <i class="fa fa-trash"></i> </button>
                                @endforeach
                            @endif

                        </th>
                        <th><input type="text" class="form-control" wire:change="changekey('unit',$event.target.value,{{$loop->index}})"> </th>
                        <th><textarea class="form-control" wire:change="changekey('normal_range',$event.target.value,{{$loop->index}})"></textarea></th>
